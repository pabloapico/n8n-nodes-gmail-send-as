...
Install the exact `.tgz` artifact generated by GitHub Actions. This avoids running repository lifecycle scripts inside the n8n container and provides a deterministic rollback target.
...
Restart n8n afterward so it reloads node types. Direct installation from the Git repository is intentionally not used for this first deployment because the repository contains TypeScript source while the `.tgz` contains the validated compiled `dist/` output.

## Uninstall / rollback

The node does not add database tables or modify existing credentials. To remove it:
...
For version rollback, install an older `.tgz` package explicitly:

```bash
npm install /path/to/n8n-nodes-gmail-send-as-<version>.tgz
```
