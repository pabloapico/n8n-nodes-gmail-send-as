...
Until the package is published to npm, a self-hosted n8n instance can install it directly from GitHub when the container has `git` available:

```bash
mkdir -p /home/node/.n8n/nodes
cd /home/node/.n8n/nodes
npm install github:pabloapico/n8n-nodes-gmail-send-as#main
```

Then restart the n8n container so it reloads node types.

A more deterministic test installation is to download the `.tgz` artifact generated by GitHub Actions and install that exact file:

```bash
cd /home/node/.n8n/nodes
npm install /path/to/n8n-nodes-gmail-send-as-0.1.0.tgz
```
